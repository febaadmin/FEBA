...
import { Home, ClipboardList, Calendar, FileText, BookOpen, MessageSquare, User, LogOut, School, Menu, X, Bell, Megaphone, Video } from "lucide-react";
...
const nav = [
  { label: "Accueil",         icon: Home,          to: "/student/home" },
  { label: "Mes notes",       icon: ClipboardList, to: "/student/grades" },
  { label: "Absences",        icon: Calendar,      to: "/student/attendance" },
  ...
  { label: "Salles virtuelles", icon: Video,       to: "/student/virtual", feature: "virtual_classrooms" },
  { label: "Bulletins",       icon: BookOpen,      to: "/student/bulletins" },
  { label: "Messages",        icon: MessageSquare, to: "/student/messages" },
  { label: "Annonces",        icon: Megaphone,     to: "/student/announcements" },
  { label: "Mon profil",      icon: User,          to: "/student/profile" },
];
...
export default function StudentLayout() {
  ...
  const { data: notifData } = useQuery({ queryKey: ["notif-count-s"], queryFn: notificationsAPI.unreadCount, refetchInterval: 30000 });
  const { data: notifListData } = useQuery({ queryKey: ["notif-list-s"], queryFn: notificationsAPI.list, enabled: notifOpen });
  const markAllMut = useMutation({ mutationFn: notificationsAPI.markAllRead, onSuccess: () => { qc.invalidateQueries({ queryKey: ["notif-count-s"] }); } });
  const { data: msgCountData } = useQuery({ queryKey: ["msg-count-s"], queryFn: conversationsAPI.unreadCount, refetchInterval: 15000 });
        ...
        {open && (
          <motion.aside key="sidebar" initial={{ x: -260 }} animate={{ x: 0 }} exit={{ x: -260 }} transition={{ duration: 0.2 }}
            ...
            <div className="p-5 flex items-center gap-3 border-b border-white/10">
              <div className="w-9 h-9 rounded-xl overflow-hidden bg-white border-2 border-violet-400 flex items-center justify-center shrink-0">
                <img src={logoSrc} alt={t("FEBA")} className="w-full h-full object-contain" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-white font-bold text-sm">{t("FEBA")}</p>
                <p className="text-slate-400 text-xs">{t("Espace Élève")}</p>
              </div>
              ...
              {visibleNav.map(item => (
                <NavLink key={item.to} to={item.to} onClick={closeSidebarOnMobile}
                  className={({ isActive }) => clsx("flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all",
                    isActive ? "text-white bg-violet-600/70 shadow-lg" : "text-slate-400 hover:text-white hover:bg-white/10")}>
                  <item.icon className="w-4 h-4 shrink-0" />
                  <span className="truncate">{t(item.label)}</span>
                  {item.to === "/student/messages" && msgUnread > 0 && (
            ...
            <div className="p-3 border-t border-white/10">
              <div className="flex items-center gap-2 px-4 py-2 mb-1">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-slate-400 text-[10px]">{t("Élève")}</p>
                </div>
              </div>
          ...
          <div className="flex items-center gap-3">
              ...
              {notifOpen && (
                <div className="absolute right-0 top-10 w-72 sm:w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                    ...
                    <button onClick={() => setNotifOpen(false)} className="text-slate-400 text-lg">×</button>
                  </div>
            ...
            <div className="flex items-center gap-2 pl-3 border-l border-slate-100">
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-xs text-slate-400">{t("Élève")}</p>
              </div>
            </div>
          </div>
        </header>
