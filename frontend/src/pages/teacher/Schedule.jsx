...
export default function TeacherSchedule() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t("Mon Emploi du Temps")} subtitle={`${schedules.length} créneau(x) cette semaine`} />

      {activeDays.length === 0 ? (
        <div className="card text-center py-16 text-slate-400">
          <Clock className="w-12 h-12 mx-auto mb-3 opacity-30" />
          <p className="font-medium">{t("Aucun créneau défini")}</p>
          <p className="text-sm mt-1">{t("L'administration n'a pas encore saisi votre emploi du temps.")}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {activeDays.map(({ day, items }, di) => (
            <motion.div key={day} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: di * 0.07 }}
              className="card">
              <p className="font-bold text-slate-800 mb-3 border-b border-slate-100 pb-2">{t(day)}</p>
                ...
                {items.map((item, ii) => (
                  <div key={item.id} className={`flex items-start gap-2 p-2.5 rounded-xl border ${COLORS[ii % COLORS.length]}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400">{t("Classe")}: {item.class_name} {item.room ? `• ${item.room}` : ""}</p>
                    </div>
                  </div>
                ))}
              </div>
