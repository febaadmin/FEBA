...
export default function AdminAttendance() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["attendance"] }); toast.success(t("Présence enregistrée !")); closeModal(); },
    onError: (e) => toast.error(extractApiError(e)),
  });
  const updateMut = useMutation({
    mutationFn: ({ id, data }) => attendanceAPI.update(id, data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["attendance"] }); toast.success(t("Modifiée !")); closeModal(); },
  });
  const deleteMut = useMutation({
    mutationFn: attendanceAPI.delete,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["attendance"] }); toast.success(t("Supprimée.")); setDeleteItem(null); },
  });
  ...
  const cols = [
    ...
    { key: "file", label: t("Pièce jointe"), sortable: false, render: r => r.justification_file ? (
      <a href={r.justification_file} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs text-primary hover:underline">
        <FileText className="w-3 h-3" />{t("Justificatif")}</a>
    ) : "—" },
  ];

  return (
    <div className="space-y-6">
      <PageHeader title={t("Absences & Présences")} subtitle={t("{n} enregistrement(s)", { n: records.length })}
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editItem ? t("Modifier") : t("Nouvelle présence")} size="md">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <textarea {...register("justification")} className="input" rows={2} placeholder={t("Motif de l'absence ou du retard…")} />
          </div>
          <div>
            <label className="label">{t("Justificatif (image, PDF, doc…)")}</label>
            <div className="flex items-center gap-3">
              ...
              <button type="button" onClick={() => fileRef.current?.click()}
                ...
                <Paperclip className="w-4 h-4" />{justFile ? t("Changer le fichier") : t("Joindre un justificatif")}
              </button>
              {justFile && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-slate-800 text-lg">{t("Détail présence")}</h2>
              <button onClick={() => setViewItem(null)} className="text-slate-400 hover:text-slate-600 text-2xl leading-none">×</button>
            </div>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between py-2 border-b border-slate-50"><span className="text-slate-500">{t("Élève")}</span><span className="font-medium">{viewItem.student_name}</span></div>
              <div className="flex justify-between py-2 border-b border-slate-50"><span className="text-slate-500">{t("Classe")}</span><span>{viewItem.student_class || "—"}</span></div>
              <div className="flex justify-between py-2 border-b border-slate-50"><span className="text-slate-500">{t("Date")}</span><span>{viewItem.date}</span></div>
              <div className="flex justify-between py-2 border-b border-slate-50"><span className="text-slate-500">{t("Statut")}</span><StatusBadge status={viewItem.status} /></div>
              {viewItem.justification && <div className="bg-slate-50 rounded-xl p-3"><p className="text-xs text-slate-500 mb-1 font-medium">{t("Justification")}</p><p className="text-sm">{viewItem.justification}</p></div>}
              {viewItem.justification_file && (
                <a href={viewItem.justification_file} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-primary-50 text-primary px-3 py-2 rounded-xl text-sm hover:bg-primary-100">
                  <FileText className="w-4 h-4" />{t("Voir le justificatif")}</a>
              )}
            </div>
          </div>
        </div>
