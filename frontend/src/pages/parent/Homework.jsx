...
export default function ParentHomework() {
  ...
  const cols = [
    ...
    { key: "att", label: t("Pièces jointes"), sortable: false, render: r => (
      (r.attachments?.length > 0)
        ? <span className="text-xs text-primary font-medium">{r.attachments.length} {t("fichier(s)")}</span>
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t("Devoirs")} subtitle={t("Devoirs des classes de mes enfants")} />
      <div className="card">
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2 text-xs text-slate-500">
              ...
              <span className={`px-2 py-1 rounded-lg font-medium ${new Date(selected.due_date) < new Date() ? "bg-danger-50 text-danger" : "bg-success-50 text-success"}`}>
                <CalendarClock className="w-3 h-3 inline mr-1" />{t("Rendu le")} {selected.due_date}
              </span>
            </div>
            ...
            {selected.attachments?.length > 0 && (
              <div>
                <p className="text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
                  <FileText className="w-4 h-4" />{t("Pièces jointes")}</p>
