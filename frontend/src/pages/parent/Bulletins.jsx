...
export default function ParentBulletins() {
    ...
    { key: "pdf", label: t("PDF"), accessor: "pdf_url", sortable: false,
      render: r => (r.pdf_url || r.pdf_file)
        ? <a href={r.pdf_url || r.pdf_file} target="_blank" rel="noreferrer"
            ...
            <Download className="w-3 h-3" />{t("Télécharger")}</a>
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t("Bulletins scolaires")} subtitle={t("Bulletins de mes enfants")} />
      ...
      {bulletins.length === 0 && !isLoading ? (
        <div className="card text-center py-12 text-slate-400">
          ...
          <p>{t("Aucun bulletin disponible pour cette période")}</p>
        </div>
      ) : (
